...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = streamText({
      ...
      tools: {
        web_search: {
          description: 'Search the web for current information about companies, markets, products, and competitive intelligence. Use this tool when you need recent, factual information.',
          ...
          execute: async ({ query }: { query: string }) => {
            try {
              ...
              console.log('Tavily results:', data.results?.length || 0, 'items');
...
CRITICAL: You have access to the web_search tool. USE IT to search for:
- Team Wendy's official website (teamwendy.com)
- Recent news about Team Wendy
...
CRITICAL: USE the web_search tool to find Team Wendy's current product lineup and offerings.
...
CRITICAL: USE the web_search tool to research Team Wendy's customer base and target markets.
...
CRITICAL: USE the web_search tool to find information about Team Wendy's global presence and distribution.
...
CRITICAL: USE the web_search tool to verify Team Wendy's market presence and activity.
...
CRITICAL: USE the web_search tool to research market trends, emerging opportunities, and competitive landscapes.
